...
const ThankfulPerson2 = props => {
  ...
  const address3 = useValidation('', validator.address3(isAddress));
  ...
  if (isName) {
    nameInvalid = (name.value === '' && isNameReq) || name.value.length > 50;
    payload.thankful2_name = name.value;
  }

  if (isTelNo) {
    contactInvalid =
      (phone.value === '' && isTelNoReq) || phone.value.length > 12;
          ...
          {isTelNo && (
            <div className="row mb-2">
              ...
              <div className="col-7 pr-4 text-left">
                ...
                {phone.value.length > 0 && <ErrorFormatted {...phone.error} />}
              </div>
            </div>
          )}
